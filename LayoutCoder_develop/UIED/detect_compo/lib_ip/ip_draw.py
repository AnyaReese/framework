...
def draw_bounding_box(org, components, color=(0, 255, 0), line=1,
                      show=False, write_path=None, name='board', is_return=False, wait_key=0, write_blank_path=None):
    """
    ...
    red_color = (255, 0, 0)
    blank_board = np.full_like(org.copy(), background_color, dtype=np.uint8)
    for compo in components:
        bbox = compo.put_bbox()
        board = cv2.rectangle(board, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, line)
        blank_board = cv2.rectangle(blank_board, (bbox[0], bbox[1]), (bbox[2], bbox[3]), red_color, line)
    if show:
        cv2.imshow(name, board)
    ...
    if write_path is not None:
        ...
        # board = board[100:-110]
        cv2.imwrite(write_path, board)
        cv2.imwrite(write_blank_path, blank_board)
    return board
